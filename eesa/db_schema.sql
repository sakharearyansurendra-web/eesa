...
-- ---------------------------------------------------------------
-- USERS & ACCESS LEVELS
-- role determines what an approved user can do.
--   super_admin      -> full control of everything, manages other admins
--   admin            -> manage announcements, activities, gallery, team
--   aptitude_manager -> only manages aptitude results/question papers
--   member           -> logged-in student, no back-office access (default for joiners)
-- status tracks the join workflow.
-- ---------------------------------------------------------------
CREATE TABLE users (
  id INT AUTO_INCREMENT PRIMARY KEY,
  full_name VARCHAR(120) NOT NULL,
  email VARCHAR(150) NOT NULL UNIQUE,
  username VARCHAR(50) UNIQUE DEFAULT NULL,       -- assigned on approval
  password_hash VARCHAR(255) DEFAULT NULL,          -- set on approval
  role ENUM('super_admin','admin','aptitude_manager','member') NOT NULL DEFAULT 'member',
